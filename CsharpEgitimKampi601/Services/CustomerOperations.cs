...
namespace CsharpEgitimKampi601.Services
{
	public class CustomerOperations
	{
		...
		{
			var connection = new MongoDbConnection(); //bağlantı adresi ve veritabanını adlık
			var customerCollection = connection.GetCustomersCollection(); //koleksiyonumuza ulaştık

			var document = new BsonDocument //ekleme işlemi için gerekli olan parametreleri eklyeceğim
			{
			...
			customerCollection.InsertOne(document); //customerCollection'ın içine ekleme işlemimizi yaptık
		}

		public List<Customer> GetAllCustomer() 
		{
			var connection = new MongoDbConnection(); //veri tabanına erişim sağla
			var customerCollection = connection.GetCustomersCollection(); //koleksiyona erişim sağla

			var customers = customerCollection.Find(new BsonDocument()).ToList(); //koleksiyon içindeki verileri hafızaya aldık
			List<Customer> customerList = new List<Customer>();
			foreach (var c in customers) //35. satırdaki verileri adım adım işleyeceğiz
			{
